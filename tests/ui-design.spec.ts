...
test.describe("Desktop UI Structure", () => {
  ...
  test.beforeEach(async ({}, testInfo) => {
    testInfo.setTimeout(4 * 60_000) // 4 minutes for AI model response
  })

  test("Desktop UI Structure", async ({ page }) => {
    /**
    ...
    await runSteps({
      ...
      steps: [{ description: "Click Add your first product button to expand" }]
    })
    await expect(page.getByRole("main")).toHaveScreenshot(
      screenshotFilename({
